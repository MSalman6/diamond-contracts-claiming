...
async function main() {
    ...
    let currentPotBN = hardhat_1.ethers.utils.parseEther(currentPot.toString());
    ...
    // // never claimed:      15%
    const cryptoJS = new cryptoJS_1.CryptoJS();
    const claim1Address1 = "0x69d1521d584e4F011A3ee4F620759aDAB758333b";
    const claim1AddressOld = "dDZuUpUDjbSxyufLJS1FkWxToq9k41dcAJ";
    const claim1AddressOldRipe = (0, cryptoHelpers_1.ensure0x)(cryptoJS.dmdAddressToRipeResult(claim1AddressOld));
    ...
    const claim2AddressOldRipe = (0, cryptoHelpers_1.ensure0x)(cryptoJS.dmdAddressToRipeResult(claim2AddressOld));
    ...
    const claim3AddressOldRipe = (0, cryptoHelpers_1.ensure0x)(cryptoJS.dmdAddressToRipeResult(claim3AddressOld));
    ...
    logResult('add1:', await (await claimContract.addBalance(claim1AddressOldRipe, { value: hardhat_1.ethers.utils.parseEther(claim1Value.toString()) })).wait(1));
    logResult('add2:', await (await claimContract.addBalance(claim2AddressOldRipe, { value: hardhat_1.ethers.utils.parseEther(claim2Value.toString()) })).wait(1));
    logResult('add3:', await (await claimContract.addBalance(claim3AddressOldRipe, { value: hardhat_1.ethers.utils.parseEther(claim3Value.toString()) })).wait(1));
}
